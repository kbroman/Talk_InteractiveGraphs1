...
var dat = [ 23.78, 28.677, 25.292, 26.731, 27, 27.821, 28.406, 20.756, 21.481,
...
function densityEstimate(data, bw, xMin, xMax, nPoints)
{
  var xy = new Array(2);
  for(var i=0; i < nPoints; i++) {
    xy[i] = [xMin + (xMax-xMin)*i/nPoints, 0];
  }
  for(var i=0; i<nPoints; i++) {
    for(var j=0; j < data.length; j++) {
      xy[i][1] += dnorm((xy[i][0]-dat[j])/bw)/bw/data.length;
    }
  }
  return xy;
}
...
var onedigit = d3.format(".1f");
  ...
  var path = svg.append("svg:path")
       ...
       .attr("stroke-width", 3)
       .attr("d", line(densityEstimate(dat, bandwidth, xMin, xMax, nPoints)));
  ...
  d3.select("input[type=range]").on("change", function() {
       ...
       path.transition().attr("d", line(densityEstimate(dat, bandwidth, xMin, xMax, nPoints)));
